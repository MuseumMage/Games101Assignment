...
{
    // point
    Vector3f p (static_cast<float>(x), static_cast<float>(y), 0);
    ...
    // cross product
    float a_z = ab.cross(ap).z();
    float b_z = bc.cross(bp).z();
    float c_z = ca.cross(cp).z();

    return (a_z > 0 && b_z > 0 && c_z > 0) || (a_z < 0 && b_z < 0 && c_z < 0);
}
    ...
    for (int i = x_min; i <= x_max; i++)
    {
        for (int j = y_min; j <= y_max; j++) {
            if (insideTriangle(i, j, t.v)) {
                // calculate z value, pixel value is i and j
                ...
                if (z_interpolated < depth_buf[index]) {
                    ...
                    set_pixel(Eigen::Vector3f(static_cast<float>(i), static_cast<float>(j), 0), t.getColor());
                }
            }
        }
    }
